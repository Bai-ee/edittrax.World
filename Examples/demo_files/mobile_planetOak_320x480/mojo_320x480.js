(function() {
    ...
    function Creative(dmo){
        ...
        function init(wrapperID) {
            var subdirectory = "150103_OPS_PlanetOat";
            ...
            var config = {
                ...
                paths: {},
                bundles: {
                    "Rosetta":["core.pack","ad.pack","cnvr.usweb.pack","cnvr.mojo.pack","filters.pack","hammer.pack","tweenmax.pack","rotatorslide.pack"]
                }
            };
        ...
        function createElements(){
            ...
            var borders = {
                l:R.create("div").set({width:"1px", height:height, backgroundColor:borderColor, left:0, top:0, zIndex:119,  pointerEvents:"none", parentNode:stage}).render(),
                r:R.create("div").set({width:"1px", height:height, backgroundColor:borderColor, right:0, top:0, zIndex:119, pointerEvents:"none",parentNode:stage}).render(),
                t:R.create("div").set({width:width, height:"1px", backgroundColor:borderColor, left:0, top:0, zIndex:119, pointerEvents:"none", parentNode:stage}).render(),
                b:R.create("div").set({width:width, height:"1px", backgroundColor:borderColor, left:0, bottom:0, zIndex:119, pointerEvents:"none", parentNode:stage}).render()
            };
			...
			var nav_prev_img = R.create("div").set({
				css:{
					backgroundImage: R.create("var").set({name:"nav_prev_img", defaultValue:"nav_prev_img.png", dataType:"String", required:false, exposed:true}).render().value(),
					backgroundSize: "contain",
					backgroundPosition: "center center",
					left: 0,
					top: 259,
			...
			var nav_next_img = R.create("div").set({
				css:{
					backgroundImage: R.create("var").set({name:"nav_next_img", defaultValue:"nav_next_img.png", dataType:"String", required:false, exposed:true}).render().value(),
					backgroundSize: "contain",
					backgroundPosition: "center center",
					left: 278,
					top: 259,
			...
			/* [CREATING_COMPONENTS] */
			var catImgVarsArr = [
					R.create("var").set({name:"cat_img1", defaultValue:"Broad_Audience_Image_f1.png", dataType:"String", required:true, exposed:true}).render().value(),
					R.create("var").set({name:"cat_img2", defaultValue:"Broad_Audience_Image_f2.png", dataType:"String", required:false, exposed:true}).render().value(),
					//MAINTENANCE
					//add the third cat image on line 478
				];

			var headlineArr = [
				R.create("var").set({name:"headline_1", defaultValue:"Broad_Audience_Headline_f1_b.png", dataType:"String", required:false, exposed:true}).render().value(),
				R.create("var").set({name:"headline_2", defaultValue:"Broad_Audience_Headline_f2_b.png", dataType:"String", required:false, exposed:true}).render().value(),
				R.create("var").set({name:"headline_3", defaultValue:"Broad_Audience_Headline_f3_b.png", dataType:"String", required:false, exposed:true}).render().value(),
			];	
			...
			for (var i = 0; i < headlineArr.length; i++) {
				...
				rosetta:{
					parentNode:stage,
					tint: R.create("var").set({name:"headline_img2_tint", defaultValue:"", dataType:"Color", required:false, exposed:true}).render().value()
				},
				attr:{
					id: "cat_img" + (i + 1)
				}
				...
				headImgsArr.push(headline_img);

				if(i === 2){
				
				const thirdImage = R.create('div').set({
				    css: {
				      backgroundImage: R.create("var").set({name:"gifCont", defaultValue:"Broad_Audience_Image_f3.png", dataType:"String", required:false, exposed:true}).render().value(),
				      height: height,
				      backgroundSize: "contain",
					  backgroundPosition: "center center",
				      width: width,
				...
				const gifCont = R.create('div').set({
				    css: {
				      backgroundImage: R.create("var").set({name:"gifCont", defaultValue:"Product_Animation_f3.gif", dataType:"String", required:false, exposed:true}).render().value(),
				      height: height,
				      width: width,
				      // backgroundSize: "contain",
					  backgroundPosition: "center center",
				    ...
				    rosetta: {
				      parentNode: thirdImage
				    }
				  }).render();

				catImgsArr.push(thirdImage);
			...
			console.log(headlineArr.length + " This is headline ar")
			console.log(catImgsArr.length + " This is cat ar")
			console.log(headImgsArr.length);

			var rotatorSlide;
			var rotatorSlideText;
			...
			function startRotator() {
				...
				rotatorSlideText = R.create("rotatorslide").set({
					id: "rotatorSlide",
					container: rotatorSlideContainer,
					direction: "horizontal",
					width: 320,
				...
				nav_next_img.on("click", rotatorSlide.showNext);
				nav_prev_img.on("click", rotatorSlide.showPrevious);

				if (rotatorSlide.elements.length <= 1){
					TweenMax.set([nav_next_img.element, nav_prev_img.element], {autoAlpha: 0});
				}
				rotatorSlide.controlledBy = [rotatorSlideText];
				rotatorSlideText.controlledBy = [rotatorSlide];
			};

			/* [BATCH_LOADING] */
			var requiredArr = [logo_img];
			var allElementsArr = [fg_img, style_img, cta_img, nav_prev_img, nav_next_img, bg_img, bg_color, rotatorSlideContainer];
			...
			var megaBatch = R.create("batch")
				...
				.require(headImgsArr, 1)
				.add(allElementsArr)
				.add(headImgsArr, 1)
				.render({
					success: function(){
						hideFailed(R.filter.removeParallel(catImgsArr,headImgsArr,R.filter.parallel(catImgsArr,headImgsArr)));
						displayLoaded([
							R.filter.success(requiredArr),
							R.filter.success(headImgsArr),
			...
			var hit_area = R.create("div").set({id:"ad_hit", width: width, height: height, pointerEvents: "auto", cursor: "pointer", zIndex:50, parentNode:stage});
            ...
            function animateElements() {
            	...
            	tl.timeScale( 1.35 )
            	tl.to(stageBlock.element,.25,{autoAlpha:0},0);
            	tl.to(rotatorSlideContainer.element,0.8,{left:0, ease:"Power2.easeOut"},.3);
            	tl.from(headImgsArr[0].element,.8,{left:80},.5);
            	tl.from([nav_next_img.element, nav_prev_img.element],0.25,{autoAlpha:0},1);
            	tl.from(logo_img.element,.25,{autoAlpha:0},.25);
            	tl.from(cta_img.element,.25,{autoAlpha:0, onComplete:startAutoPlay},1.1);

            	console.log(rotatorSlideText);
            	console.log(rotatorSlide);
