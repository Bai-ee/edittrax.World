(function() {
    ...
    function Creative(dmo){
        ...
        function init(wrapperID) {
            ...
            req( ['require'].concat(config.bundles.Rosetta), function() {
                ...
                Platform.overwrite({
                    ...
                    placementWidth:Number(dmo.mediaWidth) || 1024,
                    placementHeight:Number(dmo.mediaHeight) || 769,
        ...
        function createElements(){
            ...
            var borders = {
                l:R.create("div").set({width:"1px", height:height, backgroundColor:borderColor, left:0, top:0, zIndex:1351,  pointerEvents:"none", parentNode:stage}).render(),
                r:R.create("div").set({width:"1px", height:height, backgroundColor:borderColor, right:0, top:0, zIndex:1351, pointerEvents:"none",parentNode:stage}).render(),
                t:R.create("div").set({width:width, height:"1px", backgroundColor:borderColor, left:0, top:0, zIndex:1351, pointerEvents:"none", parentNode:stage}).render(),
                b:R.create("div").set({width:width, height:"1px", backgroundColor:borderColor, left:0, bottom:0, zIndex:1351, pointerEvents:"none", parentNode:stage}).render()
            };
			...
			for (var i = 0; i < catImgVarsArr.length; i++) {
				...
				var headline_text = R.create("div").set({
					...
					attr:{
						...
						textContent: R.create("var").set({name:"headline_text" + (i + 1), defaultValue:"Every family tree needs roots. Islamorada, in the Florida Keys." /*+ (i + 1)*/, required:false, exposed:true}).render().value()
					},
					data:{
						hitIndex: i + 1
					}
