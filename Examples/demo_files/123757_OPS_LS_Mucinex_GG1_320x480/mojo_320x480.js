(function() {
    ...
    function Creative(dmo){
        ...
        function init(wrapperID) {
            var subdirectory = "132757_OPS_Mucinex";
            ...
            req( ['require'].concat(config.bundles.Rosetta), function() {
                ...
                if (R.isCompatible === true) {
                    ...
                    Platform.overwrite({
                        ...
                        FOFVersion: R.create("var").set({name:"fof_version", dataType:"String", defaultValue:"2.1.4", exposed:false}).render().value(),
        ...
        function createElements(){
            ...
            var borders = {
                l:R.create("div").set({width:"1px", height:height, backgroundColor:borderColor, left:0, top:0, zIndex:678,  pointerEvents:"none", parentNode:stage}).render(),
                r:R.create("div").set({width:"1px", height:height, backgroundColor:borderColor, right:0, top:0, zIndex:678, pointerEvents:"none",parentNode:stage}).render(),
                t:R.create("div").set({width:width, height:"1px", backgroundColor:borderColor, left:0, top:0, zIndex:678, pointerEvents:"none", parentNode:stage}).render(),
                b:R.create("div").set({width:width, height:"1px", backgroundColor:borderColor, left:0, bottom:0, zIndex:678, pointerEvents:"none", parentNode:stage}).render()
            };
...
//CUSTOM WORK
          ...
          var red_line_right = R.create("div").set({
            css:{
              backgroundColor: "#cc1631",
            ...
            rosetta:{
              parentNode:stage,
              tint: R.create("var").set({name:"red_line_img_tint", defaultValue:"", dataType:"Color", required:false, exposed:true}).render().value()
            },
            attr:{
              id: "red_line_img"
            }
  ...
  //
			...
			var red_line_img = R.create("div").set({
				css:{
//CUSTOM
					backgroundColor: "#cc1631",
//
...
//
          ...
          height: 3,
			...
			var pill_img = R.create("div").set({
				css:{
					backgroundImage: R.create("var").set({name:"pill_img", defaultValue:"pill_img.png", dataType:"String", required:false, exposed:true}).render().value(),
					backgroundSize: "contain",
					backgroundPosition: "center center",
					left: 0,
					top: 0,
				...
				rosetta:{
					parentNode:stage,
					tint: R.create("var").set({name:"pill_img_tint", defaultValue:"", dataType:"Color", required:false, exposed:true}).render().value()
				},
				attr:{
					id: "pill_img"
				}
...
//CUSTOM
        stageBlock, red_line_right,
//

        fg_img, style_img, headline_img, subhead_img, red_line_img, cta_img, pill_img, gradient_img, bg_img, bg_color];
 ...
 //CUSTOM
              if(stageBlock.element){all_frames.set(stageBlock.element, {autoAlpha:0}, 0)};
              if(logo_img.element){all_frames.from(logo_img.element, .5, {autoAlpha:0}, 0)};
              if(headline_img.element){all_frames.from([red_line_img.element,red_line_right.element], 0.75, {width: 0, ease: 'Power2.easeOut'}, 1)};
              if(cta_img.element){all_frames.from(cta_img.element, .6, {autoAlpha: 1, y:-600, ease: 'Elastic.easeOut', easeParams:[.5,1]}, 1.7)};
              if(pill_img.element){all_frames.from(pill_img.element, .6, {autoAlpha: 1, y:-600, ease: 'Elastic.easeOut', easeParams:[1,1]}, 1.9)};
//
              if(headline_img.element){all_frames.from(headline_img.element, 0.75, {left: -250, ease: 'Power2.easeOut'}, 0)};
              if(subhead_img.element){all_frames.from(subhead_img.element, 0.75, {left: 300, ease: 'Power2.easeOut'}, 0.5)};
