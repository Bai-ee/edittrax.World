(function() {
    ...
    function Creative(dmo){
        ...
        function createElements(){
            ...
            var borders = {
                l:R.create("div").set({width:"1px", height:height, backgroundColor:borderColor, left:0, top:0, zIndex:518,  pointerEvents:"none", parentNode:stage}).render(),
                r:R.create("div").set({width:"1px", height:height, backgroundColor:borderColor, right:0, top:0, zIndex:518, pointerEvents:"none",parentNode:stage}).render(),
                t:R.create("div").set({width:width, height:"1px", backgroundColor:borderColor, left:0, top:0, zIndex:518, pointerEvents:"none", parentNode:stage}).render(),
                b:R.create("div").set({width:width, height:"1px", backgroundColor:borderColor, left:0, bottom:0, zIndex:518, pointerEvents:"none", parentNode:stage}).render()
            };
...
///////////////////////////////////////////////////
	...
	fontSize: R.create("var").set({name:"headline_text_font_size", defaultValue:"40", dataType:"Sring", required:false, exposed:true}).render().value(),
	...
	letterSpacing: R.create("var").set({name:"headline_text_tracking", defaultValue:"1", dataType:"String", required:false, exposed:true}).render().value(),					
	...
	border: R.create("var").set({name:"cta_text_border", defaultValue:"", dataType:"String", required:false, exposed:true}).render().value(),
					...
					padding: R.create("var").set({name:"cta_text_padding", defaultValue:"7px 15px 7px 15px", dataType:"String", required:false, exposed:true}).render().value(),
					left: 26,
					top: 423,
					width: 268,
					height: 30,
			...
			var group1Array = [headline_text, cta_text];
            ...
            function animateElements() {
                ...
                var all_frames = new TimelineMax()
					...
					if(headline_text.element){all_frames.from(headline_text.element, 2, {y:-10, autoAlpha: 0, ease: 'Power1.easeOut'}, 1.5)};
					if(cta_text.element){all_frames.from(cta_text.element, 2, {y:-10, autoAlpha: 0, ease: 'Power1.easeOut', onComplete:hoverOn}, 2.5)};
                    if(lifestyle_img.element){all_frames.from(lifestyle_img.element, 2, {y:-10,autoAlpha: 0, ease: 'Power1.easeOut'}, 0.5)};
                    if(lifestyle_img.element){all_frames.from(gradient_vertical_color.element, 2, {y:-10,autoAlpha: 0, ease: 'Power1.easeOut'}, 3.5)};
