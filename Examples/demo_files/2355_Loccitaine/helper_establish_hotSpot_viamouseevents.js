    ...
    function Creative(dmo){
        ...
        function init(wrapperID) {
            ...
            var config = {
                ...
                paths: {},
                bundles: {
                    "Rosetta":["core.pack","ad.pack","cnvr.advantage.pack","cnvr.usweb.pack","filters.pack","xmlpush.pack","alignmentgroup.pack","hammer.pack","tweenmax.pack","rotatorfade.pack"]
                }
            };
        ...
        function createElements(){
            ...
            var borders = {
                l:R.create("div").set({width:"1px", height:height, backgroundColor:borderColor, left:0, top:0, zIndex:1412,  pointerEvents:"none", parentNode:stage}).render(),
                r:R.create("div").set({width:"1px", height:height, backgroundColor:borderColor, right:0, top:0, zIndex:1412, pointerEvents:"none",parentNode:stage}).render(),
                t:R.create("div").set({width:width, height:"1px", backgroundColor:borderColor, left:0, top:0, zIndex:1412, pointerEvents:"none", parentNode:stage}).render(),
                b:R.create("div").set({width:width, height:"1px", backgroundColor:borderColor, left:0, bottom:0, zIndex:1412, pointerEvents:"none", parentNode:stage}).render()
            };
...
//
          ...
          border: R.create("var").set({name:"cta_text_border_color", defaultValue:"", dataType:"String", required:false, exposed:true}).render().value(),
					padding: R.create("var").set({name:"cta_text_padding", defaultValue:"7px 11px 7px 11px", dataType:"String", required:false, exposed:true}).render().value(),
					left: 39,
					top: 337,
					width: 227,
					height: 27,
...
//CUSTOM
            ...
            if (!has_cattext && has_pricetext){
              cat_text_line1_color.top = 465;
              price_text.marginTop = 25;
...
//
        ...
        lineBotRotator.controlledBy = [lineTopRotator, priceTextRotator, catTextRotator, rotatorFade];
			...
			var group1Array = [headline_text, subhead_text, cta_text];
			var allElementsArr = [
...
//CUSTOM
        ...
        catCont,
//
        fg_img, style_img, nav_prev_img, nav_next_img, lifestyle_img, bg_img, style_bar_color, logo_bg_color, text_frame_color, /*cat_text_line1_color, cat_text_line2_color,*/ cat_overlay_color, bg_color, rotatorFadeContainer];
...
//
            ...
            function animateElements() {
              ...
              tl.from(lifestyle_img.element, 3.5, {force3D:true, x:xLeft, y:yTop, scale:1.2, opacity:0, ease:easeType, yoyo:false, repeat:0},"-=1.9")
              ...
              tl.from(nav_next_img.element, 2, {autoAlpha:0,x:20 },1.4)
              tl.from(nav_prev_img.element, 2, {autoAlpha:0,x:-20, onComplete:pointerEventsOn},1.4)

              TweenMax.set(catCont.element, {autoAlpha:0});
              TweenMax.set(cat_overlay_color.element, {autoAlpha:0});

              var cta_hitArr = [rotatorFadeContainer, hit_area, nav_prev_img, nav_next_img];
              var catContHover = false;
              var hitAreaHover = true;

              var rotY = parseInt(rotatorFadeContainer.height) - height;
              var rotX = parseInt(rotatorFadeContainer.width) - width;
              ...
              function onMouseUpdate(e) {
                ...
                if (y < height && x < width) {
                  ...
                  TweenMax.to(lifestyle_img.element, .5, {force3D:true, scale:1.02})
                  if((x > rotLeft && x < rotRight) && (y > rotTop && y < (height - 20))){
                    console.log("inside rotator hot spot");
                    TweenMax.to([catCont.element, cat_overlay_color.element],.25, {autoAlpha:1, ease:"Power2.easeOut"});
                  }
                } else {
                  TweenMax.to(lifestyle_img.element, .5, {force3D:true, scale:1.0})
                }
                // else {
                //   console.log("outisde rotator hot spot");
                //   catContHover = false;
                //   hitAreaHover = true;
              ...
              var rotTop = height - parseInt(rotatorFadeContainer.height);
              var rotLeft = (width - parseInt(rotatorFadeContainer.width)) / 2;
              var rotRight = rotLeft + parseInt(rotatorFadeContainer.width);
              var rotBot = ((height - rotTop) - parseInt(rotatorFadeContainer.height));
              ...
              cta_hitArr.forEach(function(hit, i){
                  hit.on("mouseenter", function() {
                    if (i == 1 && !catContHover){
                      console.log("test");
                      TweenMax.to(lifestyle_img.element, .5, {force3D:true, scale:1.02})
                      catContHover = true;
                    } else if (i == 0 && hitAreaHover) {
                      console.log("test");
                      TweenMax.to([catCont.element, cat_overlay_color.element],.25, {autoAlpha:1, ease:"Power2.easeOut"});
                      TweenMax.to(lifestyle_img.element, .5, {force3D:true, scale:1.02})
                      catContHover = false;
                    } else if (i == 2 && hitAreaHover || i == 3 && hitAreaHover){
                      console.log("test");
                      TweenMax.to(lifestyle_img.element, .5, {force3D:true, scale:1.02})
                    }
                  });
                  ...
                  hit.on("mouseout", function() {
                    if (i == 1 && catContHover){
                      ...
                      // if((x > rotLeft && x < rotRight) && (y > rotTop && y < height)){
                      //   console.log("rotator hot spot");
                      //   TweenMax.to([catCont.element, cat_overlay_color.element],.25, {autoAlpha:0, ease:"Power2.easeOut"});
                      //   TweenMax.to(lifestyle_img.element, .5, {force3D:true, scale:1.02})
                      // } else {
                    ...
                    // else if  ((i == 0 && hitAreaHover) && ((x > rotLeft && x < rotRight) && (y > rotTop && y < height))){
                    //   console.log("rotator hot spot");
                    //   TweenMax.to([catCont.element, cat_overlay_color.element],.25, {autoAlpha:0, ease:"Power2.easeOut"});
                    //   TweenMax.to(lifestyle_img.element, .5, {force3D:true, scale:1})
                    // }
                  });
              });
