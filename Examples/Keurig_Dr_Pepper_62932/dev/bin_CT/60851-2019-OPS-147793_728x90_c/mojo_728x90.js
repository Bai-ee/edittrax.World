(function() {
    ...
    function Creative(dmo){
        ...
        function createElements(){
            ...
            var borders = {
                l:R.create("div").set({width:"1px", height:height, backgroundColor:borderColor, left:0, top:0, zIndex:133,  pointerEvents:"none", parentNode:stage}).render(),
                r:R.create("div").set({width:"1px", height:height, backgroundColor:borderColor, right:0, top:0, zIndex:133, pointerEvents:"none",parentNode:stage}).render(),
                t:R.create("div").set({width:width, height:"1px", backgroundColor:borderColor, left:0, top:0, zIndex:133, pointerEvents:"none", parentNode:stage}).render(),
                b:R.create("div").set({width:width, height:"1px", backgroundColor:borderColor, left:0, bottom:0, zIndex:133, pointerEvents:"none", parentNode:stage}).render()
            };
...
///////////////////////////////////////////////////
			...
			var f1_copy_img = R.create("div").set({
				css:{
					backgroundImage: R.create("var").set({name:"f1_copy_img", defaultValue:"f1_copy_img.png", dataType:"String", required:true, exposed:true}).render().value(),
					backgroundSize: "contain",
					backgroundPosition: "center center",
					left: 0,
					top: 0,
				...
				rosetta:{
					parentNode:stage,
					tint: R.create("var").set({name:"f1_copy_img_tint", defaultValue:"", dataType:"Color", required:false, exposed:true}).render().value()
				},
				attr:{
					id: "f1_copy_img"
				}
            ...
            function animateElements() {
                var all_frames = new TimelineMax()	
					...
					if(car_img.element){all_frames.fromTo(car_img.element, 6, {y:-parseInt(car_img.height)}, {y:height+80},0)};
					if(map_img.element){all_frames.to(map_img.element, 3, {top: -390}, 1.3)};			

					// //turn right
					if(car_img.element){all_frames.to(car_img.element, 0.1, {rotation: -90},1.5)};
					if(car_img.element){all_frames.to(car_img.element, 1, {left:"+638"},1.6)};
					if(car_img.element){all_frames.to(car_img.element, 2, {y:"-50"},1.6)};

					// //turn left
					if(car_img.element){all_frames.to(car_img.element, 0.1, {rotation: 0},2.5)};
					// if(car_img.element){all_frames.to(car_img.element, 1, {left:"+640"},1.6)};
					if(car_img.element){all_frames.to(car_img.element, 2, {y:"90"},2.8)};

					// //turn up
					// if(f2_bottles_img.element){all_frames.to(f2_bottles_img.element, 1, {left: -350}, 3.5)};
					// if(car_img.element){all_frames.to(car_img.element, 0.1, {rotation: -90},3.5)};
					// if(map_img.element){all_frames.to(map_img.element, 2, {x: "-577"}, 3.5)};
					// if(car_img.element){all_frames.to(car_img.element, 2.1, {x:"343"},3.6)};
					// if(car_img.element){all_frames.to(car_img.element, 0.4, {y:"0"},3.6)};

					// // turn right
					// if(f3_copy_img.element){all_frames.from(f3_copy_img.element, .8, {left: 300}, 4)};
					// if(car_img.element){all_frames.to(car_img.element, 0.1, {rotation: 0},4.1)};
					// if(car_img.element){all_frames.to(car_img.element, 1.1, {x:"+890"},4.2)};

					if(f1_copy_img.element){all_frames.to(f1_copy_img.element, 2.2, {top: -120}, 1.3)};
					if(f2_bottles_img.element){all_frames.to(f2_bottles_img.element, 1.8, {left: "+200"}, 1.9)};
					if(f3_copy_img.element){all_frames.from(f3_copy_img.element, 1.7, {top: 90}, 1.8)};
